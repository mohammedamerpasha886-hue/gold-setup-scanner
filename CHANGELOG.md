...
## [0.8.2] - 2026-08-20
### Fixed
- **Critical candle-order bug**: TwelveData returns `values` newest-first, but the
  pipeline expects chronological order (`candles[-1]` = newest). The list was
  consumed reversed, so indicators/scan logic ran on inverted time. Now stored
  chronological. (Latent bug, now fatal since TwelveData is the sole source.)
- **Timestamp timezone**: the feed stamps bars ~UTC+10; timestamps are now
  calibrated to true UTC using the response `Date` header.
### Added
- **Staleness guard**: a frozen/2-day-old response is retried up to 3× then
  rejected with a clear error instead of silently feeding the strategy engine.
### Changed
- Free key `9f179a78fb53412cba6181812d239689` hardcoded as the only key (no env).
- 65 tests passing.
